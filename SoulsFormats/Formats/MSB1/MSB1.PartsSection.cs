        ...
        public class PartsSection : Section<Part>
        {
            ...
            public override List<Part> GetEntries()
            {
                return Util.ConcatAll<Part>(
                    MapPieces, Objects, Enemies, Players, Collisions, Navmeshes, DummyObjects, DummyEnemies, ConnectCollisions);
            }
            ...
            internal void GetNames(MSB1 msb, Entries entries)
            {
                foreach (Part part in entries.Parts)
                    part.GetNames(msb, entries);
            }

            internal void GetIndices(MSB1 msb, Entries entries)
            {
                foreach (Part part in entries.Parts)
